...
class BlackjackGame:
    ...
    def play_game(self) -> List[List[Card]]:
        ...
        for player in self._players:
            current_player_hand = self._deal_hand(player)
            all_player_hands.append(current_player_hand)
        return all_player_hands

    def _deal_hand(self, player: Player) -> List:
        current_player_hand = []
        for _ in range(2):
            self._give_new_card(player, current_player_hand)
        while self._player_score_under_22(current_player_hand) and player.wants_new_card():
            self._give_new_card(player, current_player_hand)
    ...
    @staticmethod
    def _player_score_under_22(player_cards: List[Card]) -> bool:
        """
